...
import { ArrowLeftIcon, HeartIcon, BookmarkIcon, ShoppingCartIcon } from 'lucide-react';
...
export const CookbookPage: React.FC<CookbookPageProps> = ({
  ...
  useEffect(() => {
    ...
    document.title = `${cookbook.title} | Community Cookbook`;
    ...
    descriptionMeta.setAttribute('content', cookbook.description || `Discover ${cookbook.title} by ${cookbook.author} on Community Cookbook`);
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="flex flex-col lg:flex-row gap-8">
          <div className="w-full lg:w-1/3">
            <img 
              src={cookbook.image_url || 'https://images.unsplash.com/photo-1512621776951-a57141f2eefd?ixlib=rb-1.2.1&auto=format&fit=crop&w=800&q=80'} 
              alt={cookbook.title} 
          ...
          <div className="w-full lg:w-2/3">
            ...
            <p className="text-charcoal/60 text-sm mb-6">Published {publishedDate}</p>
            <p className="text-charcoal/80 mb-6 text-lg leading-relaxed">{cookbook.description}</p>
            <div className="flex flex-wrap gap-2 mb-8">
              {tags.map(tag => <TagPill key={tag} tag={tag} />)}
            </div>
            <div className="flex flex-col sm:flex-row gap-4">
              <button className="flex items-center justify-center gap-2 px-6 py-3 bg-gray-100 rounded-sm hover:bg-gray-200 transition-colors">
                <HeartIcon size={18} />
                <span>Favorite</span>
              </button>
              <button className="flex items-center justify-center gap-2 px-6 py-3 bg-gray-100 rounded-sm hover:bg-gray-200 transition-colors">
              ...
              {cookbook.affiliate_link && (
                <a
                  ...
                  className="flex items-center justify-center gap-2 bg-coral text-white px-6 py-3 rounded-sm hover:bg-coral/90 transition-colors"
                >
        ...
        {/* Tab Navigation */}
        <div className="border-b border-gray-200 mb-8 mt-12">
        ...
        <div className="min-h-[400px]">
          {activeTab === 'reviews' && (
            <div>
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold text-charcoal">User Reviews</h2>
                <div className="flex items-center gap-3">
                  {user && !userHasReviewed && (
                    <button 
                      onClick={() => setShowReviewForm(true)}
                      className="bg-navy text-white px-6 py-2 rounded-md hover:bg-navy/90 transition-colors"
                    >
                      Write Review
                    </button>
                  )}
              ...
              {/* Review Form */}
              {showReviewForm && user && (
                <div className="bg-gray-50 rounded-md p-6 mb-8 border border-gray-200 max-w-2xl">
                  ...
                  <form onSubmit={handleReviewSubmit} className="space-y-4">
                    ...
                    <div className="flex gap-3">
                      <button
                        ...
                        className="bg-navy text-white px-6 py-2 rounded-md hover:bg-navy/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        {submittingReview ? 
                      ...
                      <button
                        ...
                        className="bg-gray-200 text-charcoal px-6 py-2 rounded-md hover:bg-gray-300 transition-colors"
                      >
                        Cancel
                      </button>
                    </div>
              ...
              {loadingReviews ? (
                ...
                <div className="text-red-600 text-center py-12 bg-red-50 rounded-md p-6">
                  {reviewsError}
                </div>
              ) : reviews.length > 0 ? (
                <div className="space-y-6 max-w-2xl">
                  {reviews.map((review) => (
                    <div key={review.id} className="border border-gray-200 rounded-md p-6 bg-gray-50">
          ...
          {activeTab === 'recipe-cards' && (
            <div>
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold text-charcoal">Recipe Cards</h2>
                {user ? (
                  <button 
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {recipeCards.map((card) => (
                    <div 
                      ...
                      className="bg-white border border-gray-200 rounded-md overflow-hidden cursor-pointer transition-all duration-300 hover:shadow-lg group"
                    >
