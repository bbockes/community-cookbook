          ...
          <button 
            ...
            className={`bg-white rounded-full p-2 shadow-md transition-all duration-200 ${
              !user 
                ? 'opacity-50 cursor-not-allowed' 
                : isWishlisted 
                  ? 'text-navy scale-110 hover:text-navy/80' 
                  : 'text-gray-400 hover:text-navy'
            }`}
          >
            <BookmarkIcon size={18} fill={isWishlisted ? 'currentColor' : 'none'} className="transition-transform duration-200 group-hover:scale-110" />
          </button>
        </div>
      </div>
    </div>;
